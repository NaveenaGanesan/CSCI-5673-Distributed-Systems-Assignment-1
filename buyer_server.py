...
def handle_login(username, password, conn,addr):
    try:
...
def handle_display_cart(buyer_id, conn, addr):
    try: 
        cart_items = customers_db.display_cart(buyer_id)
        if not cart_items:
            conn.send(f"Your cart is empty for address {addr}.".encode(FORMAT))
            return
        ...
        try:
            ...
            if msg_length:
                ...
                if msg.startswith("CREATE_ACCOUNT"):
                    ...
                    handle_login(username, password, conn, addr)
                    ...
                    handle_display_cart(buyer_id, conn, addr)
                ...
                # print(f'[{addr}] {msg}')
            # else:
            #     print(f"Error in msg length: {msg_length}")
        except Exception as e:
